...
class Compiler {
	...
	emitAssets(compilation, callback) {
		...
		const emitFiles = err => {
			...
			// 获取this.assets，this.assets: Map<name: CachedSource>
			...
			asyncLib.forEachLimit(
				...
				({ name: file, source, info }, callback) => {
					...
					const writeOut = err => {
						...
						/**  检查是否已经生成过文件。check if the target file has already been written by this Compiler*/
...
${other}`);
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			// 这里绑定了EntryPlugin，开始进行入口文件的依赖分析
