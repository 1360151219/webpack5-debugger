		...
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					...
					// 给每个Chunk一个Id：Give each selected chunk an index (to create strings from chunks)
					...
					/**
					 * 遍历chunks，获取其中的chunk，这里的chunksKey实际上就是一个Chunk。（因为chunks是一个Set，这里使用Iterator来取）
					 *
					...
					/** 获取modules中所有module的chunkKey。
					 * @description 这里简单案例中由于所有module都被打包在一个入口chunk中，因此只返回一个size=1的singleChunkSets
					 * 如果是有公共模块，则 返回有chunkSetsInGraph
					 *
					 * 返回值如下
					 * singleChunkSets：Set<chunk>
					 *
					 * chunkSetsInGraph： Map<Key, Set<Chunk>>
					 */
					...
					const addModuleToChunksInfoMap = (
						...
						// 如果没有配置name，则为空。 Determine name for split chunk
						const name =
							/** @type {string} */
					...
					for (const module of compilation.modules) {
						// Get cache group
						// L#485行：获取有效的CacheGroups，比如没有匹配到cacheGroup的缓存组过滤掉
					...
					while (chunksInfoMap.size > 0) {
						...
						// 如果ChunkInfo的总size小于minSize，则不拆。[TODO：这里的Size如何计算的]
						const enforced =
						...
						// =================兜底：去除掉chunkInfo中，module没有打包进usedChunks中的情况
						...
						if (!isReusedWithAllModules) {
							...
							// 公共modules全部链接到newChunk中，而断开跟原本用到的各个chunks的链接
							for (const module of item.modules) {
					...
					for (const chunk of Array.from(compilation.chunks)) {
						...
						// 如果 maxSizeQueueMap 没有 而且该chunk不是 async chunk，则跳过
