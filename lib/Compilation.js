...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	seal(callback) {
		...
		// 这里的 this.entries 指的是 所有入口模块。本案例中这里只有一个EntryDependency
		for (const [name, { dependencies, includeDependencies, options }] of this
			.entries) {
			// 1. 获取Chunk。entry chunk
			...
			/**
			 * 2. 创建Entrypoint
			 * Entrypoint类将所有位于单个ChunkGroup的chunks封装在一起，这些chunks表示需要为单个页面实例加载的所有bundles。
			 * 在多页面应用程序架构中，编译过程中通常会产生多个Entrypoint对象。
			 * 在单页面应用程序中，可能只包含一个Entrypoint，但可能包含许多懒加载的chunks。
			 */
			const entrypoint = new Entrypoint(options);
			if (!options.dependOn && !options.runtime) {
				entrypoint.setRuntimeChunk(chunk);
			}
			entrypoint.setEntrypointChunk(chunk); // 把刚刚的chunk添加进Entrypoint中
			this.namedChunkGroups.set(name, entrypoint);
			this.entrypoints.set(name, entrypoint);
			this.chunkGroups.push(entrypoint);
			// entrypoint 和 entry chunk 互相关联
			connectChunkGroupAndChunk(entrypoint, chunk);

			const entryModules = new Set();
			// 3. 初始化 cgc cgm & 设置 chunkGraphInit、modulesList
			for (const dep of [...this.globalEntry.dependencies, ...dependencies]) {
				entrypoint.addOrigin(null, { name }, /** @type {any} */ (dep).request);
				// dep -> module
				const module = this.moduleGraph.getModule(dep);
				if (module) {
					// 构建ChunkGraph中的 cgc 和 cgm & 初始化 chunkGraphInit
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		// 创建同步、异步chunkGroup和chunks并且连接起来
		...
		// 【重点】这个Hook绑定了比较多。splitChunksPlugin
				...
				err => {
					...
					this.codeGeneration(err => {
						...
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							this.createModuleAssets(); // 不知道干嘛的？
	...
	codeGeneration(callback) {
		...
		// 生成jobs，包含了所有modules的module、hash、runtime等。
	...
	_runCodeGenerationJobs(jobs, callback) {
		...
		const runIteration = () => {
			...
			// 这里相当于是for of：for(const job of jobs)
			asyncLib.eachLimit(
				jobs,
...
This prevents using hashes of each other and should be avoided.`);
	...
	emitAsset(file, source, assetInfo = {}) {
		...
		// 将生成的source放入this.assets中，准备写入文件
		this.assets[file] = source;
