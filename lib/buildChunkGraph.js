...
const visitModules = (
	...
	const getBlockModules = (block, runtime) => {
		...
		const module = /** @type {Module} */ (block.getRootBlock()); // 获取最初、最头部的依赖项。
	...
	/** 异步模块的block对应的chunkGroupInfo @type {Map<DependenciesBlock, ChunkGroupInfo>} */
	...
	/** 这里是记录储存了可复用模块的 由异步依赖新创建的 ChunkGroupInfo
	 * @type {Set<ChunkGroupInfo>}
	 * */
	...
	const iteratorBlock = b => {
		// 1. We create a chunk group with single chunk in it for this Block 我们为该模块创建一个包含单个moudle的ChunkGroup
		// but only once (blockChunkGroups map)
		let cgi = blockChunkGroups.get(b); // chunkGroupInfo
	...
	const processBlock = block => {
		...
		if (blockModules !== undefined) {
			...
			// 遍历所有引用的依赖modules。 Traverse all referenced modules
			for (let i = 0; i < blockModules.length; i += 2) {
				...
				// 一般来说activeState是true，此时push进入一个新的队列 queueBuffer中
	...
	const processQueue = () => {
		while (queue.length) {
			...
			switch (queueItem.action) {
				...
				// fallthrough
				// EntryChunk 第一个入口走的是这里，connect chunkGraphModule和chunkGraphChunk， cgm 和 cgc 互相链接
				...
				// fallthrough
				// 设置当前模块在 chunkGroup 和 moduleGraph 中的 preOrderIndex，并扭转状态到LEAVE_MODULE
				case ENTER_MODULE: {
				...
				// fallthrough
				case PROCESS_BLOCK: {
					// 遍历模块所有的 同步 & 异步 依赖，并且添加到 queue & queueDelayed 中，继续回到外层 queue 遍历
					processBlock(block);
					break;
				}
	...
	const calculateResultingAvailableModules = chunkGroupInfo => {
		...
		// Create a new Set of available modules at this point 为 available modules 创建一个新的Set
	...
	/** targets: queueConnect中包含了当前的ChunkGroupInfo的异步依赖中非入口类型新建的ChunkGroup集合数组 */
	const processConnectQueue = () => {
		// Figure out new parents for chunk groups
		// to get new available modules for these children
		for (const [chunkGroupInfo, targets] of queueConnect) {
			// 1. Add new targets to the list of children
			// 1. 给父 chunkGroupInfo 设置上 children 属性：异步依赖新创建的 chunkGroupInfo
			...
			// @read：获取当前chunkGroup（非异步）中所有的modules.  这里留个问题，这些可复用的modules（availableModulesToBeMerged）到底在哪里被复用了呢？
	...
	const processChunkGroupsForMerging = () => {
		...
		// Execute the merge
		// 这里的 chunkGroupsForMerging 是储存了可复用模块的 由异步依赖新创建的 ChunkGroupInfo
	...
	while (queue.length || queueConnect.size) {
		...
		// chunkGroupsForCombining 有父chunkGroup的情况
		...
		// 在 processBlock 中所有异步模块被推入到 queueDelayed 中。当所有同步模块处理好后，就开始处理异步模块了。
		if (queue.length === 0) {
...
const connectChunkGroups = (
	...
	// 在处理异步模块的时候，储存了所有的异步模块以及其cg和cgi
...
const buildChunkGraph = (compilation, inputEntrypointsAndModules) => {
	...
	/** 存储了当前所有 ChunkGroup 的 ChunkGroupInfo 信息@type {Map<ChunkGroup, ChunkGroupInfo>} */
	const chunkGroupInfoMap = new Map();

	/** @type {Set<DependenciesBlock>} */
	const blocksWithNestedBlocks = new Set();

	// 简单情况下（比如只有一个EntryPoint的情况下），Part One是起作用的。
	// PART ONE
	...
	// PART TWO
	// 用于将所有的 block 以及异步创建的 ChunkGroup进行连接
	...
	for (const [chunkGroup, chunkGroupInfo] of chunkGroupInfoMap) {
		// 一般情况chunk.runtime是undefined，将chunkGroup的runtime赋值给chunk
		for (const chunk of chunkGroup.chunks)
